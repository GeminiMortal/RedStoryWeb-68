...
export function Sidebar({
  ...
  return <>
      {/* 移动端菜单按钮 */}
      <button onClick={toggleMobile} className="fixed top-4 left-4 z-50 md:hidden bg-gray-800 p-2 rounded-lg border border-gray-700">
      ...
      <div className={cn("fixed left-0 top-0 h-full bg-gray-800/95 backdrop-blur-sm border-r border-gray-700 flex flex-col transition-transform duration-300 ease-in-out z-50", "md:translate-x-0", isMobileOpen ? "translate-x-0" : "-translate-x-full", isCollapsed ? "md:w-16" : "md:w-64")}>
        ...
        <div className={cn("p-6 border-b border-gray-700/50 flex items-center justify-between", isCollapsed && "md:p-3")}>
          {!isCollapsed && <h1 className="text-xl font-bold text-white bg-gradient-to-r from-red-500 to-orange-500 bg-clip-text text-transparent">
              红色故事
            </h1>}
        ...
        {/* 折叠按钮 - 仅桌面端显示 */}
        <button onClick={toggleSidebar} className="absolute top-1/2 -right-3 transform -translate-y-1/2 bg-gradient-to-r from-gray-700 to-gray-600 hover:from-red-600 hover:to-red-500 text-white rounded-full p-1.5 shadow-lg border border-gray-600 transition-all duration-200 hover:scale-110 hidden md:block">
