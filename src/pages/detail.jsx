...
// @ts-ignore;
import { ArrowLeft, Clock, User, Calendar, Eye, Share2, Heart, Bookmark, BookOpen, Text, Minus, Plus, ChevronLeft, CheckCircle, Loader2, Home, RefreshCw, AlertCircle } from 'lucide-react';
// @ts-ignore;
    ...
    try {
      ...
      if (result && result.data) {
        const storyData = {
          ...
          content: storyData.content || '暂无内容',
          author: storyData.author || '佚名',
          tags: Array.isArray(storyData.tags) ? storyData.tags : [],
          views: typeof storyData.views === 'number' ? storyData.views : 0
        };
        setStory(storyData);
      ...
      <main className={cn("transition-all duration-300 ease-in-out", getMainMargin())}>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="bg-slate-800/50 backdrop-blur-sm rounded-xl p-4 flex items-center justify-between border border-slate-700/50">
            ...
            <div className="flex items-center space-x-2">
              <Button onClick={() => setFontSize('small')} variant={fontSize === 'small' ? 'default' : 'ghost'} size="sm" className={cn("px-3 py-1", fontSize === 'small' && "bg-red-500 hover:bg-red-600")}>
                小
              </Button>
              <Button onClick={() => setFontSize('medium')} variant={fontSize === 'medium' ? 'default' : 'ghost'} size="sm" className={cn("px-3 py-1", fontSize === 'medium' && "bg-red-500 hover:bg-red-600")}>
                中
              </Button>
              <Button onClick={() => setFontSize('large')} variant={fontSize === 'large' ? 'default' : 'ghost'} size="sm" className={cn("px-3 py-1", fontSize === 'large' && "bg-red-500 hover:bg-red-600")}>
                大
              </Button>
            </div>
          </div>
